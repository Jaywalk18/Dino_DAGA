        ...
        for ann in anns:
            if 'bbox' in ann and ann['area'] > 0:
                ...
                if cat_id not in self.cat_id_to_idx:
                    print(f"ERROR: Unknown category ID {cat_id} in image {img_id}")
                    continue
                label_idx = self.cat_id_to_idx[cat_id]
                if label_idx < 0 or label_idx >= 80:
                    print(f"ERROR: Invalid label index {label_idx} for category ID {cat_id}")
                    continue
